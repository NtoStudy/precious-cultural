...
// 获取分页数据
const NonHeritagePageData = ref(null)
const userNonHeritagePage = async () => {
  const res = await userNonHeritagePageApi(currentPage.value, pageSize.value, activeItemIndex.value + 1);
...
const goToDetail = (item) => {
  console.log(item);
  const path = `/menu/heritage/heritageDetail/${item}`;
  router.push(path);
};
